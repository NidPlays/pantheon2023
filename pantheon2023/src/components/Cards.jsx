              ...
              return (
                ...
                >
                  ...
                  <div className="text-lg p-2">
                    {event.event_name.split("-", 1)[0]}
                  </div>
                  <div className="text-sm text-gray-50 p-2">
                    {event.common_name}
                  </div>
                  <span className="tag m-1 inline-flex">
                    <BiCategoryAlt size={18} className="mr-1" />
                    {event.category}
                  </span>
                  {event.prize_amount["1st"] !== "NA" ? (
                    <span className="tag m-1 inline-flex">
                      <BsTrophy size={18} className="mr-1" />
                      Prize {event.prize_amount["1st"]}
                    </span>
                  ) : (
                    <div />
                  )}
                  <span className="tag m-1 inline-flex">
                    <MdAppRegistration size={18} className="mr-1" />
                    Registration {event.registration_fee}
                  </span>
                  <span className="tag m-1 inline-flex">
                    <FaPlaceOfWorship size={18} className="mr-2" />
                    {event.venue}
                  </span>
                  <div className="grid-container">
                    <Link
                      to={`/events/${event.id}`}
                      className="button-86 grid-item"
                      role="button"
                    >
                      View More
                    </Link>
