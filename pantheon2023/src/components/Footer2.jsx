...
export default function Footer2({ backgroundColor }) {
  return (
    <>
      <footer
        className={`z-0 ${backgroundColor ? backgroundColor : "bg-[#151515]"}`}
      >
        <div className="bg-gradient-to-t from-yellow-600/[.15] via-transparent">
          <div className="mx-auto max-w-7xl overflow-hidden px-6 py-8 sm:py-20 lg:px-8">
            <nav
              className="relative flex justify-center items-center flex-wrap mx-0 space-x-6 my-2.5"
              aria-label="Footer"
            >
              {navigation.main.map((item) => (
                <div key={item.name} className="pb-6">
                  <Link
                    to={item.to}
                    className="text-normal font-medium leading-6 text-gray-300 hover:text-gray-400"
                  >
                    {item.name}
                  </Link>
                </div>
